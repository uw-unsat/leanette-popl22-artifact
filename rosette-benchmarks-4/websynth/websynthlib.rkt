...
(require "dom.rkt" syntax/parse/define)
(provide (except-out (all-defined-out) tags websynth:define-symbolic)
         (rename-out [websynth:define-symbolic define-symbolic]))

(define-syntax-parser websynth:define-symbolic
  [(_ x:id type:id [depth:id]) #'(define-symbolic x type #:length depth)]
  [(_ whatever ...) #'(define-symbolic whatever ...)])
