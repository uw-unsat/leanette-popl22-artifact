...
(define (Geokit::GeoLoc_inst_success? self  #:block [BLK (void)])
	(let ()
	(return (eq? (let ([self self])(begin(define tmpname2 (Geokit::GeoLoc_inst_success (object-objectid self) )) tmpname2)) true ))))

(define-symbolic Geokit::GeoLoc_inst_success (~> integer? boolean?))
;;;RETURN VALUE:
(define b (Geokit::GeoLoc_inst_success? self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? b) (eq? b (let ([self self])(begin(define tmpname3 (Geokit::GeoLoc_inst_success (object-objectid self) )) tmpname3)) ))))
