...
(define (Unitwise::Functional_from_degre self x #:block [BLK (void)])
	(let ()
	(return (Float_inst_* (Float_inst_/ (float 5.0) (float 4.0) ) (begin
	(Float_inst_+ x (float 218.52) )
	) ))))
(define (Unitwise::Functional_to_degre self x #:block [BLK (void)])
	(let ()
	(return (Float_inst_- (Float_inst_/ (Float_inst_* (float 4.0) x ) (float 5.0) ) (float 218.52) ))))
;;;RETURN VALUE:
(define res (Unitwise::Functional_from_degre self x))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? res) (Float_inst_== x (Unitwise::Functional_to_degre self res ) ))))
