...
;;;RETURN VALUE:
(define res (Unitwise::Functional_to_degf self x))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? res) (Float_inst_== res (Float_inst_- (Float_inst_/ (Float_inst_* (float 9.0) x ) (float 5.0) ) (float 459.67) ) ))))
