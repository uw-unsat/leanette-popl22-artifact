...
(define (Money::Arithmetic_inst_negative? self  #:block [BLK (void)])
	(let ()
	(return (Integer_inst_< (let ([self self])(begin(define tmpname15 (int (Money::Arithmetic_inst_fractional (object-objectid self) )))tmpname15)) (int 0) ))))

(define-symbolic Money::Arithmetic_inst_fractional (~> integer? integer?))
;;;RETURN VALUE:
(define b (Money::Arithmetic_inst_negative? self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? b) (eq? b (Integer_inst_< (let ([self self])(begin(define tmpname16 (int (Money::Arithmetic_inst_fractional (object-objectid self) )))tmpname16)) (int 0) ) ))))
