...
(define (Date_inst_quarter self  #:block [BLK (void)])
	(let ()
	(return (Integer_inst_+ (begin
	(Integer_inst_/ (begin
	(Integer_inst_- (let ([self self])(begin(define m (int (Date_inst_month (object-objectid self) )))(assume (and (Integer_inst_>= m (int 1) ) (Integer_inst_<= m (int 12) ) )) m)) (int 1) )
	) (int 3) )
	) (int 1) ))))

(define-symbolic Date_inst_month (~> integer? integer?))
;;;RETURN VALUE:
(define q (Date_inst_quarter self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? q) (and (Integer_inst_>= q (int 1) ) (Integer_inst_<= q (int 4) ) ))))
