...
(define (synth test)
  ...
  (define-symbolic* order integer? #:length (length prog+))
      ...
      (with-handlers ([exn:fail? (const (unsat))])
        (synthesize #:forall (append (all-choices crashes) order)
                    #:guarantee (assert (=> (valid-ordering newfs prog+ order)
                                            (and (apply || allowed)
                                                 cost-constraint))))))
    (cond [(sat? S) (define S+ (evaluate prog-with-syncs (complete-solution S (symbolics prog-with-syncs))))
                    (define C+ (evaluate C (complete-solution S (symbolics C))))
                    (loop S+ C+)]
          [else     sol])))
