...
(require (rename-in racket/async-channel [async-channel-get channel-get] [async-channel-put channel-put]))
...
(define-syntax-rule (send-thing thing msg args ...)
  (send thing match-thread-receive (list 'msg args ...))
...
(define-syntax-rule (send-syncd thing msg args ...)
  (let ([c (make-async-channel)])
    (send thing match-thread-receive (list 'msg c args ...))
