...
(provide litmus-file->test compile/ppc compile/x86)
       ...
       (Thread tid
               (for/fold ([ir '()]) ([insn code])
                 (match-define (list op args)
                   (map (compose1 string-downcase string-trim)
...
;; Compile a litmus-file struct into a litmus test,
;; treating the litmus-file as an x86 litmus test.
(define (compile/x86 test)
  (define pre (litmus-file-pre test))
  (define insns (litmus-file-insns test))
  (define post (litmus-file-post test))
     ...
     (for/list ([(p tid) (in-indexed procs)])
       ...
       (Thread tid
               (for/fold ([ir '()]) ([insn code])
                 ...
                 (match (string-downcase op)
                   ["mov"
                    (match-define (list dst src) (ssplit args ","))
                    (cond
                      [(string-contains? dst "[")
                       (define n (string-index dst #\[))
                       (define addr (ssubstr dst (add1 n) (- (string-length dst) 1)))
                       (define val (string-replace src "$" ""))
                       (hash-set! mem addr val)
                       (append ir
                               (list (Write (gid) (lid) p '() (string->symbol addr) (string->number val))))]
                      [else
                       (define n (string-index src #\[))
                       (define addr (ssubstr src (add1 n) (- (string-length src) 1)))
                       (define val
                         (cond [(hash-has-key? (hash-ref post p) dst)
                                (hash-ref (hash-ref post p) dst)]
                               [(hash-has-key? mem addr)
                       ...
                       (cond [val
                              (define l (lid))
                              (hash-set! regs dst (list val '() l))
                              (append ir
                                      (list (Read (gid) l p '() (string->symbol addr) (string->number val))))]
                             [else ir])])]
                   ["mfence" (append ir (list (Fence (gid) (lid) p '() 0 0 'sync)))]
                   [_ (error 'compile/x86 "unknown instruction ~v" insn)]))))))
  (define postcond
    (for/list ([(var val) post] #:unless (hash? val))
      (list (string->symbol var) (string->number val))))
