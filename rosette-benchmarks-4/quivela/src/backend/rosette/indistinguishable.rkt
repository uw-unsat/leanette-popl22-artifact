...
(define (Equivalent ctx lhs rhs [invs (list (lambda _ #t))] [vacuity? #t])
  ...
  (define new-asserts (vc))
  (define precondition (&& (vc-assumes new-asserts) (vc-asserts new-asserts)))
  (clear-vc!)
  (assume (vc-assumes old-asserts))
  (assert (vc-asserts old-asserts))
