...
; general commands
(define (cidrv4? e)
  (and (pair? e) (rosette-eq? 'cidr (car e))))

(define (ipv4? e)
  (and (pair? e) (rosette-eq? 'ip (car e))))
...
(define (translate-then-cmd cmd)
  (cond
    ...
    [(and (command? 'local-preference cmd) (= 2 (length cmd))) 
      (list (local-preference 'set (second cmd)))]
    [(and (command? 'local-preference cmd) (= 3 (length cmd))) 
      (list (local-preference (second cmd) (third cmd)))]
...
(define (is-subnet-length? s) (rosette-eq? (car s) 'subnet-length))
(define (is-subnet-length-range? s) (rosette-eq? (car s) 'subnet-length-range))
...
(define (translate-route-filter cmd)
  (cond
   ...
   [(and (rosette-eq? (third cmd) 'upto) (is-subnet-length? (fourth cmd)))
    (route-filter (translate-cidr (second cmd))
                  (upto (second (fourth cmd)))
                  (translate-route-filter-action (cddddr cmd)))]
   [(and (rosette-eq? (third cmd) 'prefix-length-range) (is-subnet-length-range? (fourth cmd)))
    (route-filter (translate-cidr (second cmd))
...
(define (translate-from-cmd cmd) 
  (cond
    ...
    [(command? 'as-path cmd) (map as-path (listify (second cmd)))]
