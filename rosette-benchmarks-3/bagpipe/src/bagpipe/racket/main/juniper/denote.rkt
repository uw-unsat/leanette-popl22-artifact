...
(define (flow? c) (and (pair? c) (rosette-eq? 'Flow (first c))))
(define (flow-ctrl c) (second c))
(define (flow-announcement c) (third c))
(define (accepted? c) (rosette-eq? 'accept (flow-ctrl c)))
(define (rejected? c) (rosette-eq? 'reject (flow-ctrl c)))
...
(define (denote-local-preference then)
  (define action (local-preference-action then))
  (define value (local-preference-pref then))
  (cond
    [(rosette-eq? 'set action) (lambda (_) value)]
    [(rosette-eq? 'subtract action) (lambda (pref) (- pref value))]
    [else undefined]))

; returns a modified announcement and flowctrl
(define (denote-then then a)
  (cond
    [(local-preference? then) (flow 'next-then (announcement-pref-update a (denote-local-preference then)))]
  ...
  (for/all ([actions actions])
    (if (null? actions)
      ...
      (let ([f (denote-action (car actions) a)])
        (if (rosette-eq? (flow-ctrl f) continue)
...
(define (denote-policies policies a)
  (define res (denote-actions denote-policy 'accept 'next-policy policies a))
  (if (accepted? res) (flow-announcement res) #f))
