...
(define (as? c) (and (pair? c) (rosette-eq? 'AS (first c))))
(define (as-routers c) (second c))
(define (as-routers-update f as*) (as (f (as-routers as*))))
(define (as-routers-lookup as ip) (findf [lambda (r) (rosette-eq? ip (router-ip r))] (as-routers as)))

(define (router ip neighbors) `(Router ,ip ,neighbors))
(define (router? r) (and (pair? r) (rosette-eq? 'Router (first r))))
...
(define (router-neighbors-lookup r ip) (findf [lambda (n) (rosette-eq? ip (neighbor-ip n))] (router-neighbors r)))

(define (neighbor ip type import export) `(Neighbor ,ip ,type ,import ,export))
(define (neighbor? n) (and (pair? n) (rosette-eq? 'Neighbor (first n))))
...
(define (neighbor-external? n) (rosette-eq? 'external (neighbor-type n)))
(define (neighbor-internal? n) (rosette-eq? 'internal (neighbor-type n)))

(define (term froms thens) `(Term ,froms ,thens))
(define (term? c) (and (pair? c) (rosette-eq? 'Term (first c))))
(define (term-froms c) (second c))
(define (term-thens c) (third c))

; froms
(define (community community) `(Community ,community))
(define (community? c) (and (pair? c) (rosette-eq? 'Community (first c))))
(define (community-community c) (second c))

(define (as-path aspath) `(AS-Path ,aspath))
(define (as-path? c) (and (pair? c) (rosette-eq? 'AS-Path (first c))))
(define (as-path-aspath c) (second c))

(define (route-filter prefix modifier action) `(Router-Filter ,prefix ,modifier ,action))
(define (route-filter? c) (and (pair? c) (rosette-eq? 'Router-Filter (first c))))
...
(define (prefix-list? c) (and (pair? c) (rosette-eq? 'Prefix-List (first c))))
(define (prefix-list-list c) (second c))

(define (prefix-list-filter list modifier) `(Prefix-List-Filter ,list ,modifier))
(define (prefix-list-filter? c) (and (pair? c) (rosette-eq? 'Prefix-List-Filter (first c))))
...
(define (community-delete? c) (and (pair? c) (rosette-eq? 'Community-Delete (first c))))
(define (community-delete-community c) (second c))

(define (community-add community) `(Community-Add ,community))
(define (community-add? c) (and (pair? c) (rosette-eq? 'Community-Add (first c))))
(define (community-add-community c) (second c))

(define (local-preference action pref) `(Local-Preference ,action ,pref))
(define (local-preference? c) (and (pair? c) (rosette-eq? 'Local-Preference (first c))))
(define (local-preference-action c) (second c))
(define (local-preference-pref c) (third c))

(define (accept) `(Accept))
(define (accept? c) (and (pair? c) (rosette-eq? 'Accept (first c))))

(define (reject) `(Reject))
(define (reject? c) (and (pair? c) (rosette-eq? 'Reject (first c))))

(define (next-term) `(Next-Term))
(define (next-term? c) (and (pair? c) (rosette-eq? 'Next-Term (first c))))

(define (next-policy) `(Next-Policy))
(define (next-policy? c) (and (pair? c) (rosette-eq? 'Next-Policy (first c))))

; prefix compare modifiers, also: 'exact, 'longer, and 'orlonger
(define (upto bits) `(Upto ,bits))
(define (upto? c) (and (pair? c) (rosette-eq? 'Upto (first c))))
(define (upto-bits c) (second c))

(define (prefix-length-range bits1 bits2) `(Prefix-Length-Range ,bits1 ,bits2))
(define (prefix-length-range? c) (and (pair? c) (rosette-eq? 'Prefix-Length-Range (first c))))
