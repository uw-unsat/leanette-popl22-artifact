...
(define (Money::Arithmetic_inst_positive? self  #:block [BLK (void)])
	(let ()
	(return (Integer_inst_> (let ([self self])(begin(define tmpname13 (int (Money::Arithmetic_inst_fractional (object-objectid self) )))tmpname13)) (int 0) ))))

(define-symbolic Money::Arithmetic_inst_fractional (~> integer? integer?))
;;;RETURN VALUE:
(define b (Money::Arithmetic_inst_positive? self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? b) (eq? b (Integer_inst_> (let ([self self])(begin(define tmpname14 (int (Money::Arithmetic_inst_fractional (object-objectid self) )))tmpname14)) (int 0) ) ))))
