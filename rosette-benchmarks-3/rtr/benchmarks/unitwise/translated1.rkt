...
(define (Unitwise::Functional_to_cel self x #:block [BLK (void)])
	...
	(return (Float_inst_- x (float 273.15) ))))
;;;RETURN VALUE:
(define res (Unitwise::Functional_from_cel self x))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? res) (Float_inst_== x (Unitwise::Functional_to_cel self res ) ))))
