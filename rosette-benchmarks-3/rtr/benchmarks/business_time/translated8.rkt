...
(define (Date_inst_fiscal_year_quarter self  #:block [BLK (void)])
	(let ()
	(return (Integer_inst_+ (begin
	(Integer_inst_/ (begin
	(Integer_inst_- (let ([tmp1 (Date_inst_fiscal_year_month self )]) (begin (assume (not (stuck? tmp1))) tmp1)) (int 1) )
	) (int 3) )
	) (int 1) ))))
...
;;;RETURN VALUE:
(define fyq (Date_inst_fiscal_year_quarter self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? fyq) (and (Integer_inst_>= fyq (int 1) ) (Integer_inst_<= fyq (int 4) ) ))))
