...
(define (BusinessTime::BusinessHours_inst_non_negative_hours? self  #:block [BLK (void)])
	(let ()
	(return (Integer_inst_>= (object-@hours self) (int 0) ))))

;;;RETURN VALUE:
(define b (BusinessTime::BusinessHours_inst_non_negative_hours? self ))

;;;VERIFIED ASSERTION:
(verify #:assume (assert (and )) #:guarantee (assert (unless (stuck? b) (if b (Integer_inst_>= (object-@hours self) (int 0) ) (void)))))
